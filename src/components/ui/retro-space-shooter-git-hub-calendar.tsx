...
export const GithubCalendar = memo(function GithubCalendar({
  ...
  return (
    <div
      className={cn(
        "w-fit mx-auto transition-all duration-500",
        gameActive ? "bg-black border border-neutral-800 rounded-lg p-2 shadow-2xl shadow-emerald-950/20" : "border-none bg-transparent",
        className,
      )}
    >
      <div className="w-fit mx-auto max-w-full flex flex-col gap-3 p-3">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-y-4 gap-x-4 mt-2">
          {/* legend (left) */}
          {showLegend && (
          ...
          {/* stats line (right) */}
          {showStats && (
            <div className="flex flex-wrap justify-end ml-auto text-sm font-sans tracking-wide">
              <a
                href={`https://github.com/${username}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-x-1 text-neutral-600 dark:text-neutral-500 select-none no-underline"
              >
                <span className="font-bold text-black" style={{ color: "#000000" }}>
                  {username}
                </span>
                <span className="text-neutral-500">contributed</span>
                <span className="font-extrabold text-[#216e39] dark:text-[#39d353]">
                  {stats.total.toLocaleString()}
                </span>
                <span className="text-neutral-500">this year on</span>
                <span className="font-bold text-black underline decoration-black underline-offset-4" style={{ color: "#000000" }}>
                  GitHub
                </span>
              </a>
            </div>
